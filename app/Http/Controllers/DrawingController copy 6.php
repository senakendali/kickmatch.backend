    ...
    foreach ($classes as $classId) {
        ...
        while ($matchCount > 0) {
            ...
            while ($participants->count() > 1) {
                ...
                if (!$p2 || $p2->id === null) {
                    // Jika peserta mendapatkan BYE, langsung masuk ke babak berikutnya tanpa pertandingan
                    $newRoundParticipants->push($p1);
                } else {
                    // Jika pertandingan normal, buat match dan pemenang masuk ke ronde berikutnya
                    $match = TournamentMatch::create([
                        'tournament_id' => $validated['tournament_id'],
                        'match_category_id' => $validated['match_category_id'],
                        'age_category_id' => $validated['age_category_id'],
