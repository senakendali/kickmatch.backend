    ...
    public function store(Request $request)
{
    ...
    foreach ($classes as $classId) {
        ...
        $round = 1;
        $matchCount = floor($participants->count() / 2); // Hitung pertandingan berdasarkan peserta yang ada

        // Proses pertandingan untuk setiap ronde
        while ($participants->count() > 1) {
            $roundMatches = 0;
            $newRoundParticipants = collect(); // Untuk menyimpan peserta yang lolos ke ronde berikutnya

            while ($participants->count() > 1 && $roundMatches < $matchCount) {
                $p1 = $participants->pop();
                ...
                // Cek ID peserta untuk pertandingan
                ...
                // Jika p2 adalah BYE, langsung menang untuk p1
                if (str_starts_with($p2->id, "BYE")) {
                    $match->update(['winner_id' => $p1_id]);
                    $newRoundParticipants->push($p1); // P1 yang menang karena BYE
                } else {
                    $newRoundParticipants->push((object) ['match_id' => $match->id]);
                }

                // Jika p1 adalah BYE, langsung menang untuk p2
                if (str_starts_with($p1->id, "BYE")) {
                    $match->update(['winner_id' => $p2_id]);
                    $newRoundParticipants->push($p2); // P2 yang menang karena BYE
                }
            ...
            $round++;
            $matchCount = floor($participants->count() / 2); // Perbarui match count berdasarkan peserta yang tersisa
        }
    }
